 ...
 */
class Lazy_Blocks_Pro_Assets {
	/**
	 * Init hooks.
	 */
	public static function init_hooks() {
		add_action( 'enqueue_block_editor_assets', 'Lazy_Blocks_Pro_Assets::enqueue_runtime' );
	}
	...
	public static function is_webpack_hmr_support() {
		return file_exists( lazy_blocks_pro()->plugin_path() . 'build/runtime.js' );
	}
	...
	public static function get_asset_file( $filepath ) {
		$asset_path = lazy_blocks_pro()->plugin_path() . $filepath . '.asset.php';
	...
	public static function register_script( $name, $path, $in_footer = true ) {
		...
		wp_register_script(
			$name,
			lazy_blocks_pro()->plugin_url() . $path . '.js',
	...
	public static function register_style( $name, $path ) {
		...
		wp_register_style(
			$name,
			lazy_blocks_pro()->plugin_url() . $path . '.css',
			array(),
			$style_data['version']
		);
	}
...
Lazy_Blocks_Pro_Assets::init_hooks();
